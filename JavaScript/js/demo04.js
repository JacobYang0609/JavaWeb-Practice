...
function delImg(){
    if (event && event.srcElement && event.srcElement.tagName == "IMG"){
        ...
        if (window.confirm("Do you want to delete the current row?")){
...
function editPrice(){
    if (event && event.srcElement && event.srcElement.tagName == "TD"){
        ...
        //目的时判断当前priceTD有子节点，而且第一个子节点是文本节点，TextNode对应的是3 ElementNode对应的是1
        if (priceTD.firstChild && priceTD.firstChild.nodeType == 3){
                ...
                if (input.tagName == "INPUT"){
                    ...
                    //6.在输入框上绑定键盘摁下事件，此处需要保证用户输入的是数字
...
function checkInput(){
    ...
    // enter: 13
    // space : 32
    //console.log(kc)：检查键盘按下的console值
    //console.log(kc);
...
function updatePrice(){
    if (event && event.srcElement && event.srcElement.tagName == "INPUT"){
        ...
        //更新当前行的小计这一个格子的值
        updateTotal(priceTD.parentElement);
    }
}
...
function updateOverall(){
    ...
    for (var i = 1; i < rows.length-1 ; i++){
        var tr = rows[i];
        var total = parseInt(tr.cells[3].innerText);
        sum = sum + total;
    }
